...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    // Placeholder drawing function for failed loads
    const drawPlaceholder = (x: number, y: number, width: number, height: number) => {
      ctx.fillStyle = 'red'; // Or load a fallback image
      ctx.fillRect(x, y, width, height);
      ctx.fillStyle = 'white';
      ctx.font = '30px Arial';
      ctx.fillText('Failed to load', x + 10, y + height / 2);
    };
    ...
    Promise.all(
      this.items.map(
        item =>
          new Promise<HTMLImageElement | null>(resolve => {
            ...
            img.onerror = () => resolve(null); // Handle error by resolving with null
            img.src = item.image;
          })
      )
    ).then(images => {
      images.forEach((img, i) => {
        ...
        if (img) {
          ...
          drawPlaceholder(x, y, cellSize, cellSize);
        }
      });
  ...
  private render(): void {
    ...
    gl.uniformMatrix4fv(
      ...
      false,
      this.worldMatrix as unknown as Float32Array
    );

    gl.uniformMatrix4fv(
      this.discLocations.uViewMatrix,
      false,
      this.camera.matrices.view as unknown as Float32Array
    );

    gl.uniformMatrix4fv(
      this.discLocations.uProjectionMatrix,
      false,
      this.camera.matrices.projection as unknown as Float32Array
    );
    gl.uniform3f(
