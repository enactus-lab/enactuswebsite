...
const handleMenuItemClick = (link: string) => {
  ...
  // Hash links (About, Contact) — SAME PAGE
  if (link.startsWith("#")) {
  ...
  className={`sm-scope z-40 ${
    isFixed
      ? "fixed top-0 left-0 w-full h-full overflow-visible pointer-events-none"
      : "w-full h-full"
        ...
        >
          ...
          >
            <Image
              src={logoUrl}
              alt="Logo"
              className="block h-10 w-auto object-contain sm:h-12 md:h-16 lg:h-20"
              draggable={false}
              width={170}
              height={50}
            />
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <button
                      type="button"
                      className="sm-panel-item relative text-black font-semibold text-[4rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block pr-[1.4em]"
